...
import img2 from '../../../images/pet6.jpg';
...
export default function DetailNew() {
  ...
  const listBread = [{ name: 'Trang chủ', link: '/' }, { name: 'Tin tức', link: '/ListNews' }, { name: data?.name }];
  const { id } = useParams();
  useEffect(() => {
    window.scrollTo(0, 0);
    newApi
        ...
        <Grid container spacing={3}>
          <Grid item lg={8} md={8} sm={12}>
            <div className="new">
              ...
              <div className="new-content">{renderHtml(data ? data.content : '')}</div>
